...
必要なもの：**Go 1.25+**、**Ollama**、**Docker**（[Oracle AI Database 26ai Free](https://www.oracle.com/database/free/) 用）。
...
5. ONNX 埋め込みモデルをダウンロードして配置
6. `picooraclaw setup-oracle` でスキーマと ONNX 埋め込みモデルを初期化
...
| `nvidia` | NVIDIA ホストの OpenAI 互換モデル | [build.nvidia.com](https://build.nvidia.com/) |
| `vllm` | セルフホスト vLLM 互換エンドポイント | 独自の API base |
| `moonshot` | Moonshot/Kimi モデル | [platform.moonshot.cn](https://platform.moonshot.cn/) |
| `github_copilot` | GitHub Copilot プロバイダー | GitHub Copilot entitlement |
...
`gateway` コマンドで PicoOraClaw を Telegram、WhatsApp、Feishu、Discord、MaixCam、QQ、DingTalk、Slack、LINE、OneBot、Web に接続できます。
...
| `dsn` | `PICO_ORACLE_DSN` | — | ADB ウォレットレス DSN |
| `wallet_path` | `PICO_ORACLE_WALLET_PATH` | — | ADB mTLS ウォレットディレクトリ |
| `pool_max_open` | `PICO_ORACLE_POOL_MAX_OPEN` | `10` | 最大オープン DB 接続数 |
| `pool_max_idle` | `PICO_ORACLE_POOL_MAX_IDLE` | `2` | 最大アイドル DB 接続数 |
| `onnx_model` | `PICO_ORACLE_ONNX_MODEL` | `ALL_MINILM_L12_V2` | 埋め込み用 ONNX モデル |
| `agent_id` | `PICO_ORACLE_AGENT_ID` | `default` | マルチエージェント分離キー |
| `embedding_provider` | `PICO_ORACLE_EMBEDDING_PROVIDER` | `api` | API キーがある場合は `api`、それ以外は ONNX |
| `embedding_api_base` | `PICO_ORACLE_EMBEDDING_API_BASE` | — | OpenAI 互換 embeddings API base |
| `embedding_api_key` | `PICO_ORACLE_EMBEDDING_API_KEY` | — | Embeddings API キー |
| `embedding_model` | `PICO_ORACLE_EMBEDDING_MODEL` | `embedding-3` | Embeddings API モデル |
...
- 設定と `PICO_ORACLE_PASSWORD` またはセットアップスクリプト引数のパスワードが一致しているか？
...
- Ollama、OpenRouter、Anthropic、OpenAI、Gemini、DeepSeek、Groq、NVIDIA、vLLM、Moonshot、GitHub Copilot プロバイダー
- Oracle AI Database + AI ベクトル検索（384次元 ONNX 埋め込み）
- チャットチャネル: Telegram、WhatsApp、Feishu、Discord、MaixCam、QQ、DingTalk、Slack、LINE、OneBot、Web
- cron 式によるスケジュールタスク
- ハートビート定期タスク
