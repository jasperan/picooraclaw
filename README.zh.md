...
所需环境：**Go 1.25+**、**Ollama**、**Docker**（用于 [Oracle AI Database 26ai Free](https://www.oracle.com/database/free/)）。
...
5. 下载并暂存 ONNX 嵌入模型
6. 运行 `picooraclaw setup-oracle` 初始化 Schema 并加载 ONNX 嵌入模型
...
| `nvidia` | NVIDIA 托管的 OpenAI 兼容模型 | [build.nvidia.com](https://build.nvidia.com/) |
| `vllm` | 自托管 vLLM 兼容端点 | 自定义 API base |
| `moonshot` | Moonshot/Kimi 模型 | [platform.moonshot.cn](https://platform.moonshot.cn/) |
| `github_copilot` | GitHub Copilot 提供商 | GitHub Copilot entitlement |
...
通过 `gateway` 命令将 PicoOraClaw 连接到 Telegram、WhatsApp、飞书、Discord、MaixCam、QQ、钉钉、Slack、LINE、OneBot、Web。
...
<details>
<summary><b>WhatsApp, 飞书, MaixCam, QQ, 钉钉, Slack, LINE, OneBot, Web</b></summary>
...
| `dsn` | `PICO_ORACLE_DSN` | — | ADB 无钱包 DSN |
| `wallet_path` | `PICO_ORACLE_WALLET_PATH` | — | ADB mTLS 钱包目录 |
| `pool_max_open` | `PICO_ORACLE_POOL_MAX_OPEN` | `10` | 最大打开 DB 连接数 |
| `pool_max_idle` | `PICO_ORACLE_POOL_MAX_IDLE` | `2` | 最大空闲 DB 连接数 |
| `onnx_model` | `PICO_ORACLE_ONNX_MODEL` | `ALL_MINILM_L12_V2` | 嵌入用 ONNX 模型 |
| `agent_id` | `PICO_ORACLE_AGENT_ID` | `default` | 多 Agent 隔离键 |
| `embedding_provider` | `PICO_ORACLE_EMBEDDING_PROVIDER` | `api` | 有 API key 时用 `api`，否则使用 ONNX |
| `embedding_api_base` | `PICO_ORACLE_EMBEDDING_API_BASE` | — | OpenAI 兼容 embeddings API base |
| `embedding_api_key` | `PICO_ORACLE_EMBEDDING_API_KEY` | — | Embeddings API key |
| `embedding_model` | `PICO_ORACLE_EMBEDDING_MODEL` | `embedding-3` | Embeddings API 模型 |
...
- 配置文件与 `PICO_ORACLE_PASSWORD` 或 setup 脚本参数中的密码是否一致？
...
- Ollama、OpenRouter、Anthropic、OpenAI、Gemini、DeepSeek、Groq、NVIDIA、vLLM、Moonshot、GitHub Copilot 提供商
- Oracle AI Database + AI 向量检索（384 维 ONNX 嵌入）
- 聊天渠道：Telegram、WhatsApp、飞书、Discord、MaixCam、QQ、钉钉、Slack、LINE、OneBot、Web
- 通过 cron 表达式定时任务
- 心跳定期任务
